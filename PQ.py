...
class PQ:
    ...
    def __init__(self, cluster_bits: int, number_of_segments: int, data_length: int, data_path: str, initially_load: bool):
        ...
        if initially_load:
            ...
            self.isTrained = True
        print("PQ Initialized")

    '''
    Actually this is the first step as we are getting the records and store them
    '''

    def insert_records(self, records_np):
        # if len(records_np) == 10000:
        #     self.train_dict(records_np)
        # else:
        self.train_before_writing(records_np)
        # self.save_vectors(rows)
        # self.save_quantized_vectors(records_np)
        # self.save_centroids()
    ...
    def train_dict(self, rows):
        embed_values = np.array([record["embed"] for record in rows])
        max_length = 100000
        if max_length > len(embed_values):
            max_length = len(embed_values)
        self.train(embed_values[:max_length])
        self.trained_data_size = len(embed_values)
    ...
    def save_quantized_vectors(self, records):
        ...
        self.get_compressed_data(records)
        with open(self.data_path, 'wb') as file:
            for index, quantized_record in enumerate(self.quantized_vectors):
                ...
                binary_data = struct.pack(
                    id_size + vec_size, index, *quantized_record)
    ...
    def load_centroids(self):
        with open(self.centroids_file_path, 'rb') as file:
            data = np.fromfile(file, dtype=np.int32, count=self.number_of_segments *
                               (self.number_of_clusters)*self.segment_size)
            data = data.reshape(
    ...
    def read_data(self):
        ...
        with open(self.data_path, 'rb') as file:
            while True:
                ...
                if not chunk:
                    break
                _, *vector = struct.unpack('I' + 'i' *
                                           self.number_of_segments, chunk)
    ...
    def train(self, training_data: list):
        ...
        for index in range(self.number_of_segments):
            ...
            print(index+1)
            self.estimators[index].fit(
                training_data[:, index*self.segment_size:(index+1)*self.segment_size])
    ...
    def get_compressed_data(self, given_vector):
        ...
        for index in range(self.number_of_segments):
            ...
            current_segment = given_vector[:, index *
                                           self.segment_size:(index+1)*self.segment_size]
            result_vector[:, index] = current_estimator.predict(
                current_segment)
    ...
    def generate_query_table(self, query_vector):
        assert self.isTrained == True, "You Should Train The Models First"
        for index in range(self.number_of_segments):
            current_segment = query_vector[0, index *
                                           self.segment_size:(index+1)*self.segment_size]
            current_centroids = self.centroids[index]
            for j, centroid in enumerate(current_centroids):
                self.table[index][j] = self.calculate_distance(
                    current_segment, centroid)
    ...
    def retrieve(self, query, top_k):
        ...
        with open('saved_db.bin', 'rb') as file:
            data = np.fromfile(
                file, dtype=np.int32, count=self.trained_data_size * (self.number_of_segments + 1))
            data = data.reshape(
                (self.trained_data_size, self.number_of_segments + 1))
