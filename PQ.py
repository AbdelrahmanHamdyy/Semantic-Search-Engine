...
class PQ:
    ...
    def __init__(self,cluster_bits:int,number_of_segments:int,data_length:int):
        ...
        self.estimators = [MiniBatchKMeans(n_clusters=self.number_of_clusters,random_state=42) for _ in range(self.number_of_segments)]
        #Now We Keep The Centroids of each cluster
        self.centroids = []
        self.table = np.zeros((self.number_of_clusters,self.number_of_segments))
    '''
...
if __name__ == '__main__':
    ...
    for  training_vector in (training_vectors):
        ...
        print("Distance")
        print(pq_model.get_distance(database_vector=pq_model.get_compressed_data(training_vector)))
        print("---------------")
